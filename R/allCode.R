...
#' a combined RMD file will be generated. The latter is useful if one wants to contnue editing the file as a
...
report_IQnca <- function(reportfile,resultpaths,templatestyle="DefaultStyle.rmdt",title="Some NCA",docx=TRUE) {
  ...
  allfiles <- do.call(c,lapply(resultpaths, function (p) list.files(path = p,pattern = "*.rmd",full.names = TRUE)))
  text <- ""
...
updateFigureNumberTitle_IQdataNCA <- function (title,figurenumber,figureindex) {
...
#' @export
#' @family NCA Data Figures
figures_indiv_IQdataNCA <- function(data, figurenumber=NULL, logY=TRUE, filename="figures_individual",nindiv=1,FLAGTIME="asis",USETAD=TRUE) {
  if (!is_IQdataNCA(data)) stopIQR("data is not an IQdataNCA object")
  if (!FLAGTIME %in% c("asis","nominal","actual")) stopIQR("FLAGTIME argument should be 'asis', 'nominal', or 'actual'")
...
getColumnNames_IQparamNCA <- function () {
  spec <- getparamspec_IQdataNCA()
  spec$PKPARAMCD
}
...
getparamspec_IQdataNCA <- function () {
  data.frame(readxl::read_excel(paste0(getparamspecPath_IQdataNCA()),skip = 2),stringsAsFactors = FALSE)
}
...
plotsinglesplit_IQdataNCA <- function (d,logY=TRUE,R2ADJTHRESHOL=0.85) {
  ...
  d$CONCPLOT <- d$CONC
  if (nrow(d)==0) {
    title <- paste0("USUBJID: ", dorig$USUBJID[1],
                    "\nPROFILE: ",dorig$PROFILE[1],
                    "\nGROUP: ",dorig$GROUP[1])
    return(
      IQRggplot()+ggtitle(title,subtitle="No evaluable data for this subject")
    )
  }
  d$TIMEPLOT <- d$TIME
  d$BLLOQmethod <- "-"
  ...
  title <- paste0("USUBJID: ", d$USUBJID[1],
                  "\nPROFILE: ",d$PROFILE[1],
                  "\nGROUP: ",d$GROUP[1])
  if (!is.na(d$IGNOREI[1])) title <- paste0("IGNORED Subject: ",d$IGNOREI[1],"\n",title)
  ...
  if (!is.na(comment)) title <- paste0("Comment: ",comment,"\n",title)
  addsubtitle2 <- ""
...
#' @export
#' @family NCA Data Listings
listing_ignoredSubjectsRecords_IQdataNCA <- function (data,listingnumber=NULL,filename=NULL) {
...
#' @export
#' @family NCA Data
reportIgnoredSUMNCA_IQdataNCA <- function (data,filename=NULL) {
...
getaddcovcolumns_IQdataNCA <- function (data) {
  ...
  x <- setdiff(x,getColumnNames_IQparamNCA())
...
nca_IQdataNCA <- function (data,milliLiter=FALSE,CTLASTwinnonlinbehavior=TRUE) {
  ...
  paramSpec <- getparamspec_IQdataNCA()
  ...
  lresNCA <- lapply(seq_along(dS), function (kkk) {
    d <- dS[[kkk]]
    d <- d[!is.na(d$CONC),]
    dCalc <- data.frame(d[,c("USUBJID","IGNORNCA","SLOPEPT","COMPTYPE","TIME","CONC","DOSE","DUR","ADM","PROFTYPE","PROFILE","TAU","AUCMETHD","BLLOQ",getColumnNames_IQparamNCA())])
...
units_IQnca <- function(data,milliLiter)
{
  ...
  relativeDose <- FALSE
  if (length(strsplit(doseUnit, "/")[[1]]) != 1) {
    relativeDose <- TRUE
  }
  ...
  units$UNIT <- gsub("CLEARANCE",CLEARANCE_UNIT,units$UNIT,fixed = TRUE)
  units$UNIT <- gsub("VOLUME",VOLUME_UNIT,units$UNIT,fixed = TRUE)
  vunits <- units$UNIT
  ...
  list(
    units = vunits,
    FACTOR_CL_V = FACTOR_CL_V,
    CLPARAM = units$PKPARAMCD[units$Unit.Calculation=="CLEARANCE"],
    VPARAM = units$PKPARAMCD[units$Unit.Calculation=="VOLUME"]
  )
}
...
export_IQnca <- function (data,filename="nca_results",SIGNIF=8,TXT=FALSE,CSV=TRUE,parameterReport = "standard") {
  ...
  covdata <- data[!duplicated(data$USUBJID),covariates]
...
getADPP_IQnca <- function (pp,dataNCA) {
  ...
  covdata <- data.frame(dataNCA[!duplicated(dataNCA$USUBJID),covariates])
